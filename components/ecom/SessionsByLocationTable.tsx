...
import { LocationRow } from '@/types/shopify-analytics'
...
export default function SessionsByLocationTable({ rows, loading }: Props) {
    ...
    <div className="bg-white rounded-2xl border border-gray-100 p-5">
      ...
      <div className="space-y-3">
        {visible.map((r, i) => {
          ...
          const label = [r.country, r.state, r.city].filter(Boolean).join(' · ')
          return (
            <div key={i}>
              <p className="text-xs text-gray-500 mb-1 truncate">{label || 'Unknown'}</p>
